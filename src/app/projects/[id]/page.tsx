...
export default function ProjectPage({ params }: PageProps) {
  ...
  const exportTasks = useMutation({
    mutationFn: () =>
      apiFetch<{ export: AirtableTaskExport }>(`/api/projects/${id}/export/airtable`, {
        method: "POST",
      }),
    onSuccess: (data) => {
      const exported = data.export.created + data.export.updated;
      setExportMessage(
        `exported ${exported}/${data.export.total} tasks to Airtable` +
          (data.export.failed ? ` (${data.export.failed} failed)` : ""),
      );
    },
    onError: (err) =>
      setExportMessage(err instanceof Error ? err.message : "Airtable export failed"),
  });
  ...
  const currentMembership = currentUser
    ? project?.memberships.find((m) => m.user.id === currentUser.id)
    : null;
            ...
            <div className="flex items-start justify-between gap-4 mt-4 mb-8">
              ...
              {canExport && (
                <div className="text-right">
                  ...
                  {exportMessage && (
                    <p className="text-xs text-muted mt-2 max-w-xs">{exportMessage}</p>
                  )}
                </div>
              )}
            </div>
